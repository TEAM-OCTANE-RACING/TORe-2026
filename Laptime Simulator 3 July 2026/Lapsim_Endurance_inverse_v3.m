% =========================================================================
% MASTER ENDURANCE SIMULATION SCRIPT 
% Exports: FULL 22km Controls to Workspace
% Visuals: FLYING LAP ONLY for readability
% =========================================================================
clc; close all;
...
Par.K_us     = -0.0017;    % Understeer Gradient [rad/g]
...
fprintf('Simulating Flying Laps...\n');
% EXACTLY AS REQUESTED: Take the last index of SS and use it for FL
v_flying_start = Results_SS.v(end); 
Results_FL     = Solve_3Pass(path_s, path_k, GGV_Env, v_flying_start);

% FORCE perfect continuity at index 1 to prevent braking pass overrides
Results_FL.v(1) = v_flying_start;

Endurance_Time = Results_SS.laptime + (Num_Laps - 1) * Results_FL.laptime;

% --- 5. CALCULATE BASE ENERGY & CONTROLS ---
fprintf('\n=== STEP 4: CALCULATE BASE ENERGY & CONTROLS ===\n');
...
Controls_SS = Generate_Control_Inputs(Results_SS, Par, aero_par, path_k);
Controls_FL = Generate_Control_Inputs(Results_FL, Par, aero_par, path_k);

% --- 6. STITCH FULL ENDURANCE ARRAYS (EXPORT ONLY) ---
fprintf('\n=== STEP 5: STITCH & EXPORT FULL ENDURANCE CONTROLS ===\n');
Total_s = []; Total_Time = []; Total_v = []; Total_Power = [];
Total_Thr = []; Total_Brk = []; Total_Steer = [];
Total_X = []; Total_Y = []; 

s_offset = 0; t_offset = 0; 
for lap = 1:Num_Laps
    if lap == 1
        res = Results_SS; ctrl = Controls_SS; pwr = P_SS_kW;
    else
        res = Results_FL; ctrl = Controls_FL; pwr = P_FL_kW;
    end
    
    % Omit the last point for all but the final lap to avoid duplicate breakpoint errors in Simulink
    if lap < Num_Laps
        idx = 1:(length(res.s)-1);
    else
        idx = 1:length(res.s);
    end
    
    Total_s     = [Total_s;     res.s(idx) + s_offset];
    Total_Time  = [Total_Time;  res.time(idx) + t_offset];
    Total_v     = [Total_v;     res.v(idx)];
    Total_Power = [Total_Power; pwr(idx)];
    Total_Thr   = [Total_Thr;   ctrl.Throttle(idx)];
    Total_Brk   = [Total_Brk;   ctrl.Brake(idx)];
    Total_Steer = [Total_Steer; ctrl.Steer_Angle_Deg(idx)];
    
    Total_X     = [Total_X;     path_X(idx)];
    Total_Y     = [Total_Y;     path_Y(idx)];
    
    s_offset   = s_offset + res.s(end);
    t_offset   = t_offset + res.laptime;
end

% --- Calculate Heading (Theta) Mapped Natively to Full 22km Distance ---
dx = diff(Total_X);
dy = diff(Total_Y);
dx = [dx; dx(end)];
dy = [dy; dy(end)];
theta_0 = atan2(dy(1), dx(1));
theta = unwrap(atan2(dy, dx));
Total_Theta = theta - theta_0;

% Normalize Pedal Inputs across the entire endurance session
max_thr = max(Total_Thr); max_brk = max(Total_Brk);
if max_thr > 0, Total_Thr_Norm = Total_Thr / max_thr; else, Total_Thr_Norm = Total_Thr; end
if max_brk > 0, Total_Brk_Norm = Total_Brk / max_brk; else, Total_Brk_Norm = Total_Brk; end

% Export to Workspace
assignin('base', 'ref_s', Total_s);
assignin('base', 'path_X', Total_X);
assignin('base', 'path_Y', Total_Y);
assignin('base', 'path_Theta', Total_Theta);
fprintf('-> FULL 22km Reference arrays (ref_s, ref_X, ref_Y, ref_Theta) exported.\n');

assignin('base', 'ctrl_s', Total_s);
assignin('base', 'ctrl_Throttle', Total_Thr_Norm);
assignin('base', 'ctrl_Brake', Total_Brk_Norm);
assignin('base', 'ctrl_Steer', Total_Steer);
assignin('base', 'ctrl_Velocity', Total_v);
fprintf('-> FULL 22km Control arrays (ctrl_s, ctrl_Throttle, ctrl_Brake, ctrl_Steer, ctrl_Velocity) exported.\n');
...
fprintf('Avg Power (Full):   %.1f kW\n', mean(Total_Power));
fprintf('Max Speed:          %.1f km/h\n', max(Results_FL.v)*3.6);

% =========================================================================
% --- VISUALIZATION SECTION (FLYING LAP ONLY) ---
% =========================================================================
figure('Name', 'Flying Lap Analysis', 'Color', 'k', 'Position', [50 50 1400 900]);
t = tiledlayout(2, 3, 'TileSpacing', 'compact', 'Padding', 'compact');
c_limits  = [min(Results_FL.v)*3.6, max(Results_FL.v)*3.6]; 
...
% BULLETPROOF PATCH FIX: Replaced 'surface' to eliminate matrix dimension errors
x_coords = path_X(:)';
y_coords = path_Y(:)';
speed_color = Results_FL.v(:)' * 3.6; 

min_len = min([length(x_coords), length(y_coords), length(speed_color)]);

patch([x_coords(1:min_len), NaN], [y_coords(1:min_len), NaN], [speed_color(1:min_len), NaN], ...
      'FaceColor', 'none', ...
      'EdgeColor', 'interp', ...
      'LineWidth', 3);
...
% --- PLOT 2: SPEED TRACE ---
nexttile;
plot(Results_FL.s, Results_FL.v * 3.6, 'LineWidth', 2, 'Color', '#4DBEEE'); 
grid on; xlim([0 max(Results_FL.s)]);
set(gca, 'Color', 'k', 'XColor', 'w', 'YColor', 'w', 'GridColor', 'w', 'GridAlpha', 0.2);
ylabel('Speed [km/h]', 'Color', 'w'); 
title(['Flying Lap: ' num2str(Results_FL.laptime, '%.2f') 's'], 'Color', 'w');

% --- PLOT 3: POWER TRACE ---
nexttile;
area(Results_FL.s, P_FL_kW, 'FaceColor', '#77AC30', 'FaceAlpha', 0.5, 'EdgeColor', 'w');
grid on; xlim([0 max(Results_FL.s)]);
set(gca, 'Color', 'k', 'XColor', 'w', 'YColor', 'w', 'GridColor', 'w', 'GridAlpha', 0.2);
ylabel('Power [kW]', 'Color', 'w'); 
title(['Total 22km Energy: ' num2str(Total_Energy_kWh, '%.2f') ' kWh'], 'Color', 'w');
xlabel('Distance [m]', 'Color', 'w');

% --- PLOT 4: STEERING TRACE ---
nexttile;
plot(Results_FL.s, Controls_FL.Steer_Angle_Deg, 'LineWidth', 1.5, 'Color', '#D95319');
grid on; xlim([0 max(Results_FL.s)]);
set(gca, 'Color', 'k', 'XColor', 'w', 'YColor', 'w', 'GridColor', 'w', 'GridAlpha', 0.2);
...
set(gca, 'Color', 'k', 'XColor', 'w', 'YColor', 'w', 'GridColor', 'w', 'GridAlpha', 0.2);
scatter(Results_FL.ay_used/9.81, Results_FL.ax_used/9.81, 15, Results_FL.v*3.6, 'filled');
xlabel('Lat G'); ylabel('Long G');
title('G-G Diagram (Flying Lap)', 'Color', 'w');
xlim([-3 3]); ylim([-3 3]); colormap(gca, 'turbo'); clim(c_limits);

% =========================================================================
% --- POST-SIMULATION VELOCITY TRACKING ERROR (EVALUATES FULL 22KM) ---
% =========================================================================
if exist('out', 'var') && isfield(out, 'Longitudinal_vel')
    fprintf('\n=== COMPUTE TRACKING ERROR (FULL ENDURANCE) ===\n');
    try
        sim_time = double(squeeze(out.Longitudinal_vel.Time(:)));
        sim_vel  = double(squeeze(out.Longitudinal_vel.Data(:)));
        
        tgt_time = double(Total_Time(:));
        tgt_vel  = double(Total_v(:));
        ...
        title('Velocity Tracking: Target vs. Actual (Full 22km)', 'Color', 'w');
...
function R = Solve_3Pass(s, k, Env, v_start)
    ...
    for i = 1:N
        curv = abs(k(i));
        if curv < 1e-4
            v_apex(i) = 21.427; % RESTORED 77 km/h MAXIMUM
        else
            v_g = 12; 
            for iter=1:15
                ay_lim = interp1(Env.v, Env.ay_max, real(v_g), 'linear', 'extrap');
                v_new = sqrt(ay_lim / curv);
                v_g = 0.6*v_g + 0.4*v_new;
            end
            v_apex(i) = min(v_g, 21.427); % RESTORED 77 km/h MAXIMUM
        end
    end
