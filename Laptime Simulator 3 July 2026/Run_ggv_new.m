...
%   - Longitudinal driver maintains speed via 'vel_ggv' and 'steer_ggv'.
...
% Velocity Sweep (m/s)
velocities = linspace(1,21.438,10); 
% velocities = 12.5:0.1:12.6;

% Initialize Storage
results = struct('v', [], 'ay_max', [], 'ax_max', [], 'ax_brk', []);
fprintf('=== STARTING GGV SIMULATION (DRIVER CONTROLLED - FAST) ===\n');
...
for i = 1:length(velocities)
    ...
    % --- TEST 1: MAX BRAKING ---
    % Driver logic: Target 0 m/s to force maximum braking effort
    simIn = baseSimIn;
    simIn = simIn.setVariable('steer_ggv', 0);
    simIn = simIn.setVariable('vel_ggv', 0); 
    
    [ax_brk, ~] = Get_Peak_Explicit(simIn, v_target, 'brake', 0, current_WB);
    
    % --- TEST 2: MAX ACCEL ---
    % Driver logic: Target a much higher speed to demand full throttle
    simIn = baseSimIn;
    simIn = simIn.setVariable('steer_ggv', 0);
    simIn = simIn.setVariable('vel_ggv', v_target + 50); 
    ...
    for steer = deg2rad(2):deg2rad(2):deg2rad(30)
        simIn = baseSimIn;
        simIn = simIn.setVariable('steer_ggv', steer);
        
        % Driver logic: Maintain current target velocity while steering
        ...
        elseif ay_curr < (best_ay * 0.95) % If grip drops by more than 5%
            ...
            if drop_count >= 2 % Require two consecutive drops to prevent false stops
                fprintf('    (Peak Lat found, breaking early at steer = %.2f degrees)\n', rad2deg(steer));
                break; 
            end
        end
    end
    ...
    fprintf('  Peak Limits: %.3fG (Acc) / %.3fG (Brk) / %.3fG (Lat)\n', ...
